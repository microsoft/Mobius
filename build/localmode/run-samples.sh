...
echo "[run-samples.sh] sparkclr-submit.sh --jars $SPARKCLR_EXT_JARS --exe SparkCLRSamples.exe $SAMPLES_DIR spark.local.dir $TEMP_DIR sparkclr.sampledata.loc $SPARKCLR_HOME/data $@"
"$SPARKCLR_HOME/scripts/sparkclr-submit.sh" --jars $SPARKCLR_EXT_JARS --exe SparkCLRSamples.exe "$SAMPLES_DIR" spark.local.dir "$TEMP_DIR" sparkclr.sampledata.loc "$SPARKCLR_HOME/data" "$@"
