...
namespace Microsoft.Spark.CSharp
{
    class DStreamSamples
    {
        ...
        /// <summary>
        /// run the sample with --conf spark.mobius.streaming.parallelJobs=1 to test parallel job in UpdateStateByKey.
        /// </summary>
        [Sample("experimental")]
        internal static void DStreamParallelJobSample()
        {
            count = 0;
            ...
            StreamingContext ssc = StreamingContext.GetOrCreate(checkpointPath,
                () =>
                {
                    ...
                    var lines = context.TextFileStream(Path.Combine(directory, "test")).MapPartitionsWithIndex(new MapPartitionsWithIndexHelper(a).Execute, true);
                    lines = context.Union(lines, lines);
                    var words = lines.FlatMap(l => l.Split(' '));
                    var pairs = words.Map(w => new KeyValuePair<string, int>(w, 1));

                    var state = pairs.UpdateStateByKey<string, int, int>((iter, s) => iter.Sum() + s, 1);

                    state.ForeachRDD((time, rdd) =>
                    {
                        Console.WriteLine("-------------------------------------------");
                        Console.WriteLine("Batch: {0} {1} Accumulator value: ", count, time);

                        // drain accumulator updates
                        var lastValue = -1;
                        while (a.Value > lastValue)
                        {
                        ...
                        Console.WriteLine("{0} Batch {1} validating parallel job has been executed by checking accumulator > 0, actual value: {2}", DateTime.Now, count, a.Value);
                        Assert.Greater(a.Value, 0, "Accumulator value should be greater than 0 due to pre-run parallel job.");
                        result = "Validation passed";

                        a.Value = 0;

                        stopFileServer = count++ > 1;
                    });
            ...
            StartFileServer(directory, "words.txt", 100, ssc);
    ...
    // Use this helper class to test accumulator variable in streaming application
    [Serializable]
    internal class MapPartitionsWithIndexHelper
    {
        private Accumulator<int> a;

        internal MapPartitionsWithIndexHelper(Accumulator<int> a)
        {
            this.a = a;
        }

        internal IEnumerable<string> Execute(int pid, IEnumerable<string> iter)
        {
            a += 1;
            return iter;
        }
